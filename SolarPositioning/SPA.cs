...
namespace SolarPositioning
{
    public class SPA
    {
        ...
        public static AzimuthZenithAngle CalculateSolarPosition(DateTime date, double lat, double lon,
            double elevation, double pressure, double temperature)
        {
            // calculate Julian (ephemeris) date and millennium
            JulianDate jd = new JulianDate(date, DeltaT(date));
            double jme = jd.getJulianEphemerisMillennium();
            double jce = jd.getJulianEphemerisCentury();

            double[] lTerms = calculateLBRTerms(jme, TERMS_L);
            double lDegrees = limitDegreesTo360(toDegrees(calculateLBRPolynomial(jme, lTerms)));

            // calculate Earth heliocentric latitude, B
            double[] bTerms = calculateLBRTerms(jme, TERMS_B);
            double bDegrees = toDegrees(calculateLBRPolynomial(jme, bTerms));

            // calculate Earth radius vector, R
            double[] rTerms = calculateLBRTerms(jme, TERMS_R);
            ...
            double hDegrees = limitDegreesTo360(nuDegrees + lon - alphaDegrees);
            ...
            double x = Math.Cos(u) + elevation * Math.Cos(phi) / 6378140;
            double y = 0.99664719 * Math.Sin(u) + (elevation * Math.Sin(phi)) / 6378140;
            double deltaAlphaDegrees = toDegrees(Math.Atan2(-x * Math.Sin(xi) * Math.Sin(h), Math.Cos(delta) - x * Math.Sin(xi) * Math.Cos(h)));

            double deltaPrime = Math.Atan2((Math.Sin(delta) - y * Math.Sin(xi)) * Math.Cos(toRadians(deltaAlphaDegrees)), Math.Cos(delta) - x
                   * Math.Sin(xi) * Math.Cos(h));
        ...
        private static AzimuthZenithAngle calculateTopocentricSolarPosition(double p, double t, double phi,
             double deltaPrime, double hPrime)
        {
            ...
            double deltaEdegrees = (p / 1010) * (283 / (273 + t))
                   * (1.02 / (60 * Math.Tan(toRadians(eZeroDegrees + 10.3 / (eZeroDegrees + 5.11)))));
        ...
        private static double calculateGeocentricSunDeclination(double betaRad, double epsilonRad,
             double lambdaRad)
        {
            return Math.Asin(Math.Sin(betaRad) * Math.Cos(epsilonRad) + Math.Cos(betaRad) * Math.Sin(epsilonRad) * Math.Sin(lambdaRad));
        }

        private static double calculateGeocentricSunRightAscension(double betaRad, double epsilonRad,
                 double lambdaRad)
        {
        ...
        {
            double epsilon0 = calculatePolynomial(jd.getJulianEphemerisMillennium() / 10.0, OBLIQUITY_COEFFS);
            return epsilon0 / 3600 + deltaEpsilon;
        }

        private static double calculateApparentSiderealTimeAtGreenwich(JulianDate jd, double deltaPsi,
                 double epsilonDegrees)
        {
            double nu0degrees = limitDegreesTo360(280.46061837 + 360.98564736629 * (jd.getJulianDate() - 2451545)
                   + 0.000387933 * Math.Pow(jd.getJulianCentury(), 2) - Math.Pow(jd.getJulianCentury(), 2) / 38710000);
            return nu0degrees + deltaPsi * Math.Cos(toRadians(epsilonDegrees));
        }

        private static double calculateDeltaPsiEpsilon(double[] deltaPsiOrEpsilonI)
        {
            double sum = 0;
            foreach (double element in deltaPsiOrEpsilonI)
            {
                sum += element;
            }
            return sum / 36000000;
        ...
        private static double[] calculateDeltaPsiI(double jce, double[] x)
        {
            ...
            for (int i = 0; i < TERMS_PE.Length; i++)
            {
                ...
                deltaPsiI[i] = (a + b * jce) * Math.Sin(toRadians(calculateXjYtermSum(i, x)));
            }
            return deltaPsiI;
        }
        ...
        private static double[] calculateDeltaEpsilonI(double jce, double[] x)
        {
            ...
            for (int i = 0; i < TERMS_PE.Length; i++)
            {
                ...
                deltaEpsilonI[i] = (c + d * jce) * Math.Cos(toRadians(calculateXjYtermSum(i, x)));
            }
        ...
        private static double limitDegreesTo360(double degrees)
        {
            ...
            double limited = 360.0 * (dividedDegrees - Math.Floor(dividedDegrees));
        ...
        public static double[][][] TERMS_L =
        {
            new double[][]
            {
                new double[] {175347046.0, 0, 0}, new double[] {3341656.0, 4.6692568, 6283.07585},
                new double[] {34894.0, 4.6261, 12566.1517},
                new double[] {3497.0, 2.7441, 5753.3849}, new double[] {3418.0, 2.8289, 3.5231},
                new double[] {3136.0, 3.6277, 77713.7715},
                new double[] {2676.0, 4.4181, 7860.4194}, new double[] {2343.0, 6.1352, 3930.2097},
                new double[] {1324.0, 0.7425, 11506.7698},
                new double[] {1273.0, 2.0371, 529.691}, new double[] {1199.0, 1.1096, 1577.3435},
                new double[] {990, 5.233, 5884.927},
                new double[] {902, 2.045, 26.298}, new double[] {857, 3.508, 398.149},
                new double[] {780, 1.179, 5223.694},
                new double[] {753, 2.533, 5507.553}, new double[] {505, 4.583, 18849.228},
                new double[] {492, 4.205, 775.523}, new double[] {357, 2.92, 0.067},
                new double[] {317, 5.849, 11790.629}, new double[] {284, 1.899, 796.298},
                new double[] {271, 0.315, 10977.079},
                new double[] {243, 0.345, 5486.778}, new double[] {206, 4.806, 2544.314},
                new double[] {205, 1.869, 5573.143},
                new double[] {202, 2.458, 6069.777}, new double[] {156, 0.833, 213.299},
                new double[] {132, 3.411, 2942.463},
                new double[] {126, 1.083, 20.775}, new double[] {115, 0.645, 0.98}, new double[] {103, 0.636, 4694.003},
                new double[] {102, 0.976, 15720.839},
                new double[] {102, 4.267, 7.114}, new double[] {99, 6.21, 2146.17}, new double[] {98, 0.68, 155.42},
                new double[] {86, 5.98, 161000.69},
                new double[] {85, 1.3, 6275.96}, new double[] {85, 3.67, 71430.7}, new double[] {80, 1.81, 17260.15},
                new double[] {79, 3.04, 12036.46},
                new double[] {75, 1.76, 5088.63}, new double[] {74, 3.5, 3154.69}, new double[] {74, 4.68, 801.82},
                new double[] {70, 0.83, 9437.76},
                new double[] {62, 3.98, 8827.39}, new double[] {61, 1.82, 7084.9}, new double[] {57, 2.78, 6286.6},
                new double[] {56, 4.39, 14143.5},
                new double[] {56, 3.47, 6279.55}, new double[] {52, 0.19, 12139.55}, new double[] {52, 1.33, 1748.02},
                new double[] {51, 0.28, 5856.48},
                new double[] {49, 0.49, 1194.45}, new double[] {41, 5.37, 8429.24}, new double[] {41, 2.4, 19651.05},
                new double[] {39, 6.17, 10447.39},
                new double[] {37, 6.04, 10213.29}, new double[] {37, 2.57, 1059.38}, new double[] {36, 1.71, 2352.87},
                new double[] {36, 1.78, 6812.77},
                new double[] {33, 0.59, 17789.85}, new double[] {30, 0.44, 83996.85}, new double[] {30, 2.74, 1349.87},
        ...
        public static double toDegrees(double radValue)
        {
            return (180.0d / Math.PI) * radValue;
        }

        public static double toRadians(double degValue)
        {
            return Math.PI * degValue / 180.0;
        }
    }
